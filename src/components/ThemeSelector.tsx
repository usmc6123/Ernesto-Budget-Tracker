...
export function ThemeSelector({ currentTheme, onSelect }: ThemeSelectorProps) {
  ...
  return (
    <div className="grid grid-cols-2 gap-2 mt-4" id="theme-selector">
      {THEMES.map((theme) => {
        ...
        return (
          <button
            ...
            className={cn(
              "p-3 rounded border text-left flex flex-col justify-between transition-all",
              isSelected
                ? "bg-[var(--bg2)] border-[var(--bone)]"
                : "bg-[var(--bg)] border-[var(--line)] hover:border-[var(--muted)]"
            )}
          >
            <span className="text-[10px] font-mono tracking-wider truncate mb-2 block text-[var(--text2)]" style={{ color: isSelected ? 'var(--bone)' : 'var(--text2)' }}>
              {theme.name}
            </span>
